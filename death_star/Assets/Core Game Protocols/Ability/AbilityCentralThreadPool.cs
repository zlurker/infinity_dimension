    ...
    public void InstanceNode(int nodeId, Tuple<int, int, int> refNode) {
        ...
        if(updateNodes) {
            ...
            if(instancedNodes[nodeId] != null) {
                ...
                centralRoot.RemoveSharedInstance(instancedNodes[nodeId].Item2, instancedNodes[nodeId]);
            }

            // Adds new link
            instancedNodes[nodeId] = refNode;

            // Informs parent of instancing as well
            centralRoot = GetRootReferenceCentral(nodeId);
            centralRoot.AddSharedInstance(refNode.Item2, Tuple.Create(castingPlayer, centralId, nodeId));
        }
