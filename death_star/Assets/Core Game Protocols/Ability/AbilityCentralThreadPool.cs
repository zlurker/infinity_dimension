...
public class AbilityCentralThreadPool : IRPGeneric, ITimerCallback {
    ...
    private Dictionary<int, Dictionary<Type, Dictionary<int, HashSet<int>>>> targettedNodes;
        ...
        if(runValueChanged) {
            ...
            if(sharedInstance.ContainsKey(nodeId))
                foreach(var id in sharedInstance[nodeId]) {
                    ...
                    totalOnCalled += centralInst.RunTargettedNodes<T>(id.Item3, variableId, typeof(OnVariableChanged), value);
                }
        ...
        for(int i = 0; i < links.Length; i++) {
            ...
            if(runOnCalled) {
                ...
                if(sharedInstance.ContainsKey(nodeId))
                    foreach(var id in sharedInstance[nodeId]) {
                        AbilityCentralThreadPool centralInst = AbilitiesManager.aData[id.Item1].playerSpawnedCentrals.GetElementAt(id.Item2);
                        totalOnCalled += centralInst.RunTargettedNodes<T>(id.Item3, variableId, typeof(OnVariableCalled), var.v);
                    }
